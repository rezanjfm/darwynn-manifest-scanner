...
// This component is camera-first and renders a full-viewport video feed.
export default function BarcodeScanner({ onScan, active }: BarcodeScannerProps) {
  ...
  const handleDetected = useCallback(
    (value: string) => {
      const now = Date.now();
      // Debounce: same barcode within 2 seconds counts as one scan
  ...
  const startNativeDetector = useCallback(
    async (video: HTMLVideoElement) => {
      ...
      const formats = ["code_128", "pdf417", "qr_code", "data_matrix", "aztec", "ean_13", "ean_8"].filter(
  ...
  const startZXing = useCallback(
    async (video: HTMLVideoElement) => {
      ...
      // ZXing needs a running video element; it polls internally
      reader.decodeFromStream(stream, video, (result, err) => {
        if (result && mountedRef.current && active) {
          handleDetected(result.getText());
        }
        void err; // decode errors are normal between frames
      });
    },
    [active, handleDetected]
  );
